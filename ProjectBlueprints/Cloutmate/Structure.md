# FocusOS Structure & Feature Inventory
...
| Path | Contents |
| --- | --- |
| `FocusOS/` | Main macOS target (SwiftUI). Subfolders: `Views`, `ViewModels`, `Services`, `Models`, `Utilities`, `Config`, `Extensions`, `Shared`. |
| `FocusOSShared/` | Swift package + DocC bundle providing shared models (`FocusOSShared/FocusOSShared/Models`) and services for all targets. |
| `FocusOSMenuBar/` | Menu bar utility app with quick capture, tasks, and upcoming posts. |
| `FocusOSWidget/` | WidgetKit extension summarizing calendar/tasks/posts. |
| `FocusOSHelper/` & `FocusOSHelper/` | Background helper apps (legacy + new naming) for scheduling/publishing tasks. |
| `FocusOSTests`, `FocusOSUITests`, `FocusOSSharedTests`, `FocusOSHelperTests`, `FocusOSMenuBarTests` | Test targets. |
...
- **SwiftData Models:**
  - Workspace: `Project`, `Task`, `Note`, `Artifact`, `Reminder`, `Template`, `InboxItem`, `CalendarEvent`, etc. (in `FocusOSShared`).
  - AI & Analytics: `AIConversation`, `AIMessage`, `ConversationDigest`, `PriorityScore`, `FocusSession`, `FocusRitual`, `RitualCompletion`, `MemoryNode`, `StoryToken`, `FlowCompanionState`, `IntentClusterSummary`, `ConfidenceSnapshot`, etc. (in `FocusOS/Models`).
- **Model container:** Each target calls `SharedDataManager.createSharedModelContainer()` to attach to the shared App Group store.
- **Config:** `FocusOS/Config/AIConfig.plist` (feature flags), `Config.plist` (API keys), `ReflectionPrompts.plist` (Flow Companion), Info.plist keys for Meta/Notion/Ollama.

---

## 3. Workspace Surfaces

- **Sidebar (`FocusOS/Views/Sidebar/`)** – ARTE-aware navigation with collapsible sections, hover rails, scroll metrics, collapse toggle.
...
- **Menu Bar (`FocusOSMenuBar`)** – SwiftUI status item with `MenuBarPopoverView`, `QuickComposerView`, `TasksView`, `UpcomingPostsView`, `MenuBarSettingsView`. Uses shared models for data and `NotificationService` for local scheduling.
- **Widget (`FocusOSWidget`)** – Standard SwiftUI widget timeline; ensure `FocusOSShared.framework` is embedded and app group entitlements are set.
- **Helper (`FocusOSHelper`)** – Background publishing, login item management, deeper integration with Meta Graph/Threads (see `BUILD_AND_SIGN_MENUBAR.md`, `SIGN_MENUBAR_APP.sh`). Legacy `FocusOSHelper` remains for historical reference until fully removed.
...
- `FocusOSTests` – SwiftData/service tests (priority, analytics, migrations).
- `FocusOSUITests` – Smoke coverage for navigation flows.
...
1. **SwiftData migrations** – Add new @Model types to `FocusOSApp.sharedModelContainer` schema list and run `MigrationRunner` if shape changes.
2. **Shared components** – Prefer building UI primitives inside `FocusOSShared/FocusOSShared/UI` for reuse (Glass system, cards) rather than duplicating per target.
