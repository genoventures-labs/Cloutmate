...
struct AreaReviewDrawer: View {
    ...
    private func projectStatus(_ project: CloutmateShared.Project) -> String {
...
#Preview {
    ...
    let task1 = CloutmateShared.Task(title: "Schedule annual check-up", status: .todo, dueDate: Calendar.current.date(byAdding: .day, value: -3, to: Date()))
    task1.areaId = area.id
    let task2 = CloutmateShared.Task(title: "Plan meal prep", status: .inProgress, dueDate: Calendar.current.date(byAdding: .day, value: 3, to: Date()))
    task2.areaId = area.id
    
    let project = CloutmateShared.Project(title: "Q4 Training Plan", status: .paused)
    ...
    .modelContainer(for: [Area.self, CloutmateShared.Task.self, CloutmateShared.Project.self, Note.self])
}
