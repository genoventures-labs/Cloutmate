...
struct AreaReviewSummary {
    ...
    let pendingTasks: [CloutmateShared.Task]
    let overdueTasks: [CloutmateShared.Task]
    let stalledProjects: [CloutmateShared.Project]
...
final class AreaReviewService {
    ...
    func summary(
        for area: Area,
        tasks: [CloutmateShared.Task],
        projects: [CloutmateShared.Project],
        notes: [Note]
    ) -> AreaReviewSummary {
