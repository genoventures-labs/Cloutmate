...
actor ModelRoutingEngine {
    ...
    private var debugMode: Bool {
        ...
        return true
        #else
        return UserDefaults.standard.bool(forKey: "com.kosmicapps.cloutmate.debugMode")
        #endif
    }
    ...
    func selectImageModel() -> ModelRoutingDecision {
        // Safety guard: Force research mode off for images
        let primaryModel = ModelTierMap.imageModel()
        let secondaryModel = ModelTierMap.imageSecondaryModel()
        let fallbackModel = ModelTierMap.imageFallbackModel()
        
        if debugMode {
            print("[ModelRoutingEngine] [IMAGE] Primary: \(primaryModel), Secondary: \(secondaryModel), Fallback: \(fallbackModel)")
        }
