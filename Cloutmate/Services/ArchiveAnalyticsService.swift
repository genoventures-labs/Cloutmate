...
final class ArchiveAnalyticsService {
    ...
    func snapshot(
        ...
        let projects = (try? modelContext.fetch(FetchDescriptor<CloutmateShared.Project>())) ?? []
        ...
        // Notes
        let notes = (try? modelContext.fetch(FetchDescriptor<CloutmateShared.Note>())) ?? []
        ...
        // Artifacts
        let artifacts = (try? modelContext.fetch(FetchDescriptor<CloutmateShared.Artifact>())) ?? []
