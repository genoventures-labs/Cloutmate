...
final class ModelWarmupService {
    ...
    private let readyModelsCacheKey = "com.kosmicapps.cloutmate.modelWarmup.readyModels"
    private let lastWarmupTimestampKey = "com.kosmicapps.cloutmate.modelWarmup.lastTimestamp"
    ...
    func startWarmup(modelContext: ModelContext) async {
        ...
        // If no models are available, mark as ready anyway to allow Aurora to function
        if modelsToWarmup.isEmpty {
            print("[ModelWarmupService] No models available for warmup, proceeding anyway")
            isWarmingUp = false
            isReady = true
        ...
        for (index, model) in modelsToWarmup.enumerated() {
            ...
            do {
                ...
                // Timeout or error - mark as pending and continue
        ...
        if !pendingModels.isEmpty {
            ...
            for (index, model) in pendingModels.enumerated() {
                ...
                do {
                    ...
                    // Still failed, but continue
                    print("[ModelWarmupService] Model \(model.name) still unavailable: \(error.localizedDescription)")
                }
            }
        }
