...
final class AIPayloadContext: @unchecked Sendable {
...
enum AIDebug {
    private static let logger = Logger(subsystem: "com.kosmicapps.FocusOS", category: "AI")
...
final class AIRecallService {
    ...
    /// Register object access (when user opens/views an object)
    /// This ensures CPS scores are created and updated even when objects are just viewed
    func registerAccessed(_ object: any RecallTrackable, modelContext: ModelContext) {
        guard config.featureFlags.recallEnabled else { return }
        
        // Update CPS for accessed object (increments access count for frequency tracking)
        PriorityEngine.shared.updateScore(
            for: object.recallObjectId,
            objectType: object.recallObjectType.rawValue,
            modelContext: modelContext,
