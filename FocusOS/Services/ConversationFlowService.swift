...
final class ConversationFlowService {
    ...
    private let logger = Logger(subsystem: "com.kosmicapps.FocusOS", category: "ConversationFlow")
    ...
    private var intentCache: [UUID: ParsedIntent] = [:]
    private var intentCacheOrder: [UUID] = []
    private let maxCachedIntents = 50
    ...
    func cacheIntent(_ intent: ParsedIntent, for messageId: UUID) {
        intentCache[messageId] = intent
        intentCacheOrder.append(messageId)
        if intentCacheOrder.count > maxCachedIntents {
            let removed = intentCacheOrder.removeFirst()
            intentCache.removeValue(forKey: removed)
        }
        #if DEBUG
        logger.debug("Cached intent \(intent.id) for message \(messageId.uuidString.prefix(6))")
        #endif
    }

    func cachedIntent(for messageId: UUID) -> ParsedIntent? {
        intentCache[messageId]
    }

    func clearCachedIntent(for messageId: UUID) {
        intentCache.removeValue(forKey: messageId)
        intentCacheOrder.removeAll { $0 == messageId }
    }
    ...
    func evaluateAndRenameChat(
        intent: ParsedIntent,
        conversation: AIConversation,
        modelContext: ModelContext
    ) async {
        guard let candidateTitle = suggestedTitle(for: intent) else {
            return
        }

        let normalizedExisting = (conversation.title ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        if normalizedExisting.caseInsensitiveCompare(candidateTitle) == .orderedSame {
            return
        }
        ...
        #if DEBUG
        logger.debug("Renamed chat \(conversation.id.uuidString.prefix(6)) to \(candidateTitle)")
        #endif
    }

    private func suggestedTitle(for intent: ParsedIntent) -> String? {
        var baseName = intent.primaryName?.trimmingCharacters(in: .whitespacesAndNewlines)
        if baseName?.isEmpty ?? true, let resolved = intent.metadata.resolvedObjects.first {
        ...
        case .task: prefix = "Task"
        case .note: prefix = "Note"
        case .post: prefix = "Post"
        case .reminder: prefix = "Reminder"
        case .artifact: prefix = "Artifact"
        default: prefix = intent.object.rawValue.capitalized
        }

        var suffix: String?
        if !intent.secondaryItems.isEmpty {
            suffix = intent.secondaryItems.prefix(2).joined(separator: ", ")
        } else if !intent.metadata.tags.isEmpty {
            suffix = intent.metadata.tags.joined(separator: ", ")
        }
    ...
    func generateToneAlignedTransition(
        ...
        let personalityContext = PersonalityQuirksService.PersonalityToneContext(
            ...
            dominantCue: mapEmotionToCue(emotionalTone.primaryEmotion),
    ...
    private func calculateSassFactor(_ arteState: EmotionalState, _ tone: EmotionalSnapshot) -> Double {
        ...
        if tone.primaryEmotion == .frustrated {
