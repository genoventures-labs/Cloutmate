...
actor OllamaBridgeService {
    ...
    private func performModelWarmup(
        ...
        for (index, attempt) in attempts.enumerated() {
            ...
            let statusPrefix = await ModelTierMap.displayName(for: model)
    ...
    private func determineOptimalModel(
        ...
        return await ModelTierMap.defaultModel()
    }
...
"""
    ...
    func generateResponseWithAppContext(
        ...
        if let providedModel = model {
            ...
            let intentCluster = await payloadContext?.intentClusters?.primaryCluster
        ...
        // Get contextual adaptations (synchronous part)
            let languagePersonality = await LanguagePersonalityService.shared
            let conversationalQuirks = await ConversationalQuirksService.shared
            let personalityQuirks = await PersonalityQuirksService.shared
            let selfAwareness = await SelfAwarenessService.shared
            let contextualAdaptation = await ContextualAdaptationService.shared
            let responsePattern = await ResponsePatternService.shared
        ...
        let transitionState: ToneTransitionState? = {
            ...
            if let transitionDataString = resolvedPayloadContext?.metadata["toneTransitionState"] as? String,
...
"""
        ...
        if let resolvedPayloadContext,
           await (!resolvedPayloadContext.recall.isEmpty ||
...
"""
        ...
        let modelDisplayName = await ModelTierMap.displayName(for: modelToUse)
    ...
    private func makeOllamaRequest(prompt: String, useThinking: Bool = false, model: String? = nil) async throws -> (response: String, thinking: String?) {
        ...
        let modeDescription = await modelToUse == ModelTierMap.fallbackModel() ? "Fallback" : (useThinking ? "Analytical" : "Casual")
...
"""
    ...
    private func extractEnabledPhases(from payloadContext: AIPayloadContext?) -> [Int] {
        ...
        if let metadata = payloadContext?.metadata {
            if let memoryGraphEnabled = metadata["memoryGraphEnabled"] as? String, memoryGraphEnabled == "true" {
                phases.append(6)
            }
            if let arteEnabled = metadata["arteEnabled"] as? String, arteEnabled == "true" {
                phases.append(7)
            }
            if let ritualsEnabled = metadata["ritualsEnabled"] as? String, ritualsEnabled == "true" {
                phases.append(8)
            }
            if let predictiveCognitionEnabled = metadata["predictiveCognitionEnabled"] as? String, predictiveCognitionEnabled == "true" {
                phases.append(9)
            }
            if let temporalIntelligenceEnabled = metadata["temporalIntelligenceEnabled"] as? String, temporalIntelligenceEnabled == "true" {
                // Phase 9 extensions
            }
    ...
    private func extractEnabledFeatures(from payloadContext: AIPayloadContext?) -> [String] {
        ...
        if let metadata = payloadContext?.metadata {
            if let emotionalContinuity = metadata["emotionalContinuity"] as? String, emotionalContinuity == "enabled" {
                features.append("emotionalContinuity")
            }
            if let cpsEnabled = metadata["cpsEnabled"] as? String, cpsEnabled == "true" {
                features.append("cps")
            }
            if let focusModeEnabled = metadata["focusModeEnabled"] as? String, focusModeEnabled == "true" {
                features.append("focusMode")
            }
            if let narrativeEnabled = metadata["narrativeEnabled"] as? String, narrativeEnabled == "true" {
                features.append("narrative")
            }
            if let crossConversationEnabled = metadata["crossConversationEnabled"] as? String, crossConversationEnabled == "true" {
                features.append("crossConversation")
            }
            if let intentClusterPredictionEnabled = metadata["intentClusterPredictionEnabled"] as? String, intentClusterPredictionEnabled == "true" {
                features.append("intentClusterPrediction")
            }
            if let memoryGraphEnabled = metadata["memoryGraphEnabled"] as? String, memoryGraphEnabled == "true" {
                features.append("memoryGraph")
            }
            if let intelligenceLayerEnabled = metadata["intelligenceLayerEnabled"] as? String, intelligenceLayerEnabled == "true" {
            ...
            if let documentAnalysisEnabled = metadata["documentAnalysisEnabled"] as? String, documentAnalysisEnabled == "true" {
                features.append("documentAnalysis")
            }
            if let imageAnalysisEnabled = metadata["imageAnalysisEnabled"] as? String, imageAnalysisEnabled == "true" {
                features.append("imageAnalysis")
            }
            if let confidenceScoringEnabled = metadata["confidenceScoringEnabled"] as? String, confidenceScoringEnabled == "true" {
                features.append("confidenceScoring")
            }
            if let conversationCompressionEnabled = metadata["conversationCompressionEnabled"] as? String, conversationCompressionEnabled == "true" {
                features.append("conversationCompression")
            }
            if let cognitiveHealthEnabled = metadata["cognitiveHealthEnabled"] as? String, cognitiveHealthEnabled == "true" {
                features.append("cognitiveHealth")
            }
            if let styleAdaptationEnabled = metadata["styleAdaptationEnabled"] as? String, styleAdaptationEnabled == "true" {
                features.append("styleAdaptation")
            }
            if let mentionLinkingEnabled = metadata["mentionLinkingEnabled"] as? String, mentionLinkingEnabled == "true" {
    ...
    nonisolated private func shouldIncludeWorkloadCues(
        ...
        if let metadata = payloadContext?.metadata {
            ...
            for key in topicKeys {
                if let topicValue = metadata[key] as? String, topicValue.lowercased() == "casual" {
                    return false
                }
            }
        }
    ...
    func detectExecutionIntent(input: String, linkedContext: LinkedContext? = nil) async throws -> ExecutionIntent? {
        ...
        let prompt = """
        Analyze this user message and determine if it's a request to execute an operation in the FocusOS app.
        ...
        do {
            ...
            if let linked = linkedContext {
                intent = applyLinkedContext(intent, linkedContext: linked)
            }
    ...
    func inferPreferenceUpdate(input: String) async throws -> PreferenceUpdate? {
        ...
        let prompt = """
        Extract user preference updates from this message for FocusOS. Return ONLY JSON with any of these keys when present:
    ...
    // MARK: - Image Analysis
    // Note: Image analysis has been moved to ImageAnalysisService
    // This method is deprecated but kept for backward compatibility
    @available(*, deprecated, message: "Use ImageAnalysisService.shared.analyzeImage instead")
    func analyzeImage(
        imageData: Data,
        mimeType: String,
        userPrompt: String?,
        ...
        return try await ImageAnalysisService.shared.analyzeImage(
            imageData: imageData,
            mimeType: mimeType,
    ...
    func analyzeDocument(
        ...
        if safePayloadContext == nil {
            safePayloadContext = await AIPayloadContext()
        }
        ...
        let fallbackModel = await ModelTierMap.documentFallbackModel()
    ...
    func makeOllamaRequestWithImages(
        model: String,
        prompt: String,
        images: [String],
        useThinking: Bool = false
    ...
    // MARK: - Linked Context Helpers
    
    private func applyLinkedContext(
        _ intent: ExecutionIntent,
        linkedContext: LinkedContext
    ) -> ExecutionIntent {
        var updated = intent
        
        if updated.projectId == nil, let projectId = linkedContext.linkedProjects.first {
            updated.projectId = projectId.uuidString
        }
        if updated.taskProjectId == nil, let projectId = linkedContext.linkedProjects.first {
            updated.taskProjectId = projectId.uuidString
        }
        if updated.taskId == nil, let taskId = linkedContext.linkedTasks.first {
            updated.taskId = taskId.uuidString
        }
        if updated.noteId == nil, let noteId = linkedContext.linkedNotes.first {
            updated.noteId = noteId.uuidString
        }
        if updated.reminderProjectId == nil, let projectId = linkedContext.linkedProjects.first {
            updated.reminderProjectId = projectId.uuidString
        }
        if updated.reminderTaskId == nil, let taskId = linkedContext.linkedTasks.first {
            updated.reminderTaskId = taskId.uuidString
        }
        if updated.artifactId == nil, let artifactId = linkedContext.linkedArtifacts.first {
            updated.artifactId = artifactId.uuidString
        }
        if updated.conversationId == nil, let conversationId = linkedContext.linkedEvents.first {
            updated.conversationId = conversationId.uuidString
        }
        
        return updated
    }
    
    // MARK: - Tool Prompt Helpers
    
    private func buildToolPrompt(
        for tool: AITool,
        input: String,
        context: String
    ) async -> String {
        ...
        case .brainstorm:
            return """
            You are Aurora, a creative strategist. Brainstorm five concise ideas that respond to the user. Avoid numbered lists unless requested.\n\(contextSection)\nUser request:\n\(input)
            """
        case .generateCaptions:
            return """
            Generate five short social captions based on the user prompt. Mix sentence lengths and avoid hashtags unless asked.\n\(contextSection)\nPrompt:\n\(input)
            """
        case .improveText:
            return """
            Improve the following text for clarity and tone. Return the improved text, then list up to three improvement notes prefixed with \"-\".\n\(contextSection)\nText:\n\(input)
            """
        case .suggestHashtags:
            return """
            Suggest up to ten relevant hashtags for the topic below. Return them on one line separated by spaces.\n\(contextSection)\nTopic:\n\(input)
            """
        case .adjustTone:
            return """
            Rewrite the text with a more polished, confident tone while preserving intent. Return only the rewritten text.\n\(contextSection)\nText:\n\(input)
            """
        }
    }
    
    private func extractImprovements(from response: String) -> [String] {
        ...
        for line in lines {
            ...
            if trimmed.hasPrefix("- ") || trimmed.hasPrefix("• ") {
                improvements.append(String(trimmed.dropFirst(2)))
            } else if let first = trimmed.first, first.isNumber,
                      let dotRange = trimmed.range(of: ". ") {
                improvements.append(String(trimmed[dotRange.upperBound...]))
            }
        }
        return improvements
    }
