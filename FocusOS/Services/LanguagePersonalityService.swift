...
final class LanguagePersonalityService {
    ...
    /// Generates a short acknowledgement using the current tone
    func conversationalAcknowledgement(
        base: String,
        tone: AuroraTone?
    ) -> String {
        ...
        let suffix: String
        switch tone {
        // Playful / uplifting vibes
        case .some(.conversational), .some(.celebratory), .some(.encouraging), .some(.rejuvenating):
            suffix = " What's next?"
        
        // Focused / steady vibes
        case .some(.deepFocus), .some(.productiveMomentum), .some(.balanced), .some(.analytical):
            suffix = " Need anything else?"
        
        // Calm / gentle vibes
        case .some(.eveningCalm), .some(.restorative), .some(.mindful), .some(.rest):
            suffix = " Just say the word if you want tweaks."
