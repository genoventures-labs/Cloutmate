...
actor CoreResponseService {
    ...
    func generateResponse(
        ...
        if let tone = toneContext {
            let promptPrefix = await AuroraToneKit.promptPrefix(for: tone)
            let toneDescription = await AuroraToneKit.summaryTone(for: tone)
    ...
    func generateResponseWithAppContext(
        ...
        if let tone = toneContext {
            ...
            if let predicted = predictedNextTone, predicted != tone {
                let predictedDescription = await AuroraToneKit.summaryTone(for: predicted)
                let predictedCategory = await AuroraToneKit.category(for: predicted)
                let currentCategory = await AuroraToneKit.category(for: tone)
            ...
            let predictedDescription = await AuroraToneKit.summaryTone(for: predicted)
            let promptPrefix = await AuroraToneKit.promptPrefix(for: predicted)
            enhancedInput = "\(promptPrefix). \(input)"
    ...
    func parsedIntent(
        for input: String,
        linkedContext: LinkedContext? = nil,
        modelContext: ModelContext? = nil
    ) async throws -> ParsedIntent? {
        return try await IntentParserService.shared.parseIntent(
            from: input,
    ...
    func cachedParsedIntent(for message: String) async -> ParsedIntent? {
        return await IntentParserService.shared.cachedIntent(for: message)
    }

    func storeParsedIntent(_ intent: ParsedIntent) async {
        await IntentParserService.shared.store(intent)
    }
    ...
    func analyzeImage(
        ...
        // Use VisionPipelineService (three-layer pipeline: QwenVisionLayer → OCRLayer → GemmaInterpretationLayer)
        let result = try await VisionPipelineService.shared.processImage(
            imageData: imageData,
            mimeType: mimeType,
        ...
        // Add tasks to summary if found
        if !result.tasks.isEmpty {
            summary += "\n\n**Tasks found:**\n" + result.tasks.enumerated().map { "\($0.offset + 1). \($0.element)" }.joined(separator: "\n")
        }
        
        // Add insights to summary if found
        if !result.insights.isEmpty {
            summary += "\n\n**Insights:**\n" + result.insights.enumerated().map { "\($0.offset + 1). \($0.element)" }.joined(separator: "\n")
        }
        
        // Add transparency messages based on PipelineContext
        var contextMessage = ""
        if result.context.ocrUsed && result.context.qwenUsed {
            contextMessage = "I combined what I saw visually with the extracted text.\n\n"
        } else if result.context.ocrUsed {
            contextMessage = "I used OCR because the vision output was incomplete.\n\n"
        } else if result.context.fallbackTriggered {
            contextMessage = "Qwen didn't give enough detail, so I filled in the gaps.\n\n"
        }
        
        return DocumentAnalysisResult(
            summary: contextMessage + summary,
    ...
    func analyzeDocument(
        ...
        var safePayloadContext = payloadContext ?? AIPayloadContext()
    ...
    func generateInsights(
        ...
        if let tone = toneContext {
            ...
            let promptPrefix = await AuroraToneKit.promptPrefix(for: defaultTone)
            _ = await AuroraToneKit.summaryTone(for: defaultTone)
