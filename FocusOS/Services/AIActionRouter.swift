...
fileprivate typealias PARATask = FocusOSShared.Task
fileprivate typealias PARANote = FocusOSShared.Note
fileprivate typealias PARAProject = FocusOSShared.Project
fileprivate typealias PARAInboxItem = FocusOSShared.InboxItem
...
struct TaskCreationRequest {
    ...
    let notes: String?
    let dueDate: Date?
    let status: FocusOSShared.TaskStatus
    let priority: FocusOSShared.TaskPriority
    let projectId: UUID?
    let areaId: UUID?
}
...
struct ProjectCreationRequest {
    let title: String
    let goal: String?
    let status: FocusOSShared.ProjectStatus
    let dueDate: Date?
    let areaId: UUID?
    let tags: [String]
}
...
struct ArtifactCreationRequest {
    let title: String
    let content: String?
    let format: FocusOSShared.OutputFormat
    let state: FocusOSShared.ArtifactState
    let tags: [String]
    let projectId: UUID?
    let areaId: UUID?
...
final class AIActionRouter {
    ...
    func route(_ action: AIIntentAction, modelContext: ModelContext) async throws -> AIActionResult {
        ...
        case let .publishPost(request):
            ...
            await memoryGraphService.syncOnUpdate(object: post, modelContext: modelContext)
            try? modelContext.save()
        ...
        case let .updateTask(request):
            ...
            if let status = request.status, status == .done {
                await memoryGraphService.syncOnTaskCompletion(task: task, modelContext: modelContext)
            } else {
                await memoryGraphService.syncOnUpdate(object: task, modelContext: modelContext)
            }
            let detail = summaryForTask(task)
        ...
        case let .deleteTask(taskId):
            ...
            await memoryGraphService.syncOnDelete(objectId: taskId, objectType: .task, modelContext: modelContext)
            return AIActionResult(
                title: action.displayName,
        ...
        case let .updateNote(request):
            ...
            await memoryGraphService.syncOnUpdate(object: note, modelContext: modelContext)
            let changeSummary = changes.isEmpty ? "No fields changed" : "Updated: \(changes.joined(separator: ", "))"
            return AIActionResult(
                title: action.displayName,
        ...
        case let .deleteNote(noteId):
            ...
            await memoryGraphService.syncOnDelete(objectId: noteId, objectType: .note, modelContext: modelContext)
            return AIActionResult(
                title: action.displayName,
        ...
        case let .updateProject(request):
            ...
            await memoryGraphService.syncOnUpdate(object: project, modelContext: modelContext)
            let detail = summaryForProject(project)
            let changeSummary = changes.isEmpty ? "No fields changed" : "Updated: \(changes.joined(separator: ", "))"
            return AIActionResult(
                title: action.displayName,
        ...
        case let .deleteProject(projectId):
            ...
            await memoryGraphService.syncOnDelete(objectId: projectId, objectType: .project, modelContext: modelContext)
            return AIActionResult(
                title: action.displayName,
        ...
        case let .createReminder(request):
            let reminder = FocusOSShared.Reminder(
                title: request.title,
                notes: request.notes,
        ...
        case let .updateArtifact(request):
            ...
            await memoryGraphService.syncOnUpdate(object: artifact, modelContext: modelContext)
            let changeSummary = changes.isEmpty ? "No fields changed" : "Updated: \(changes.joined(separator: ", "))"
            let detailLines = [
                "Title: \(artifact.title)",
                "Format: \(artifact.format.displayName)",
        ...
        case let .deleteArtifact(artifactId):
            ...
            await memoryGraphService.syncOnDelete(objectId: artifactId, objectType: .artifact, modelContext: modelContext)
            return AIActionResult(
                title: action.displayName,
        ...
        case let .duplicateProject(request):
            ...
            await memoryGraphService.syncOnCreate(object: duplicate, modelContext: modelContext)
            ...
            if request.includeTasks {
                ...
                if let tasks = try? modelContext.fetch(descriptor) {
                    for task in tasks {
                        ...
                        await memoryGraphService.syncOnCreate(object: newTask, modelContext: modelContext)
...
private func fetchArtifact(by id: UUID, context: ModelContext) -> FocusOSShared.Artifact? {
...
private func mapOutputFormat(_ raw: String?) -> FocusOSShared.OutputFormat {
    guard let raw = raw?.lowercased() else { return .brief }
    return FocusOSShared.OutputFormat(rawValue: raw) ?? .brief
}

private func mapArtifactState(_ raw: String?) -> FocusOSShared.ArtifactState {
