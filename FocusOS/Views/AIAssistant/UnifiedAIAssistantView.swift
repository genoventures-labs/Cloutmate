...
struct UnifiedAIAssistantView: View {
    ...
    @State private var showWindowSelection = false
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            let warmupService = ModelWarmupService.shared
            if !warmupService.isReady && !warmupService.isWarmingUp {
                _Concurrency.Task {
                    await warmupService.startWarmup(modelContext: modelContext)
                }
            }
        }
        .onDisappear {
        ...
        .sheet(isPresented: $showWindowSelection) {
            WindowSelectionView(
                onWindowSelected: { windowID in
                    try await handleWindowSelected(windowID)
                },
                onDismiss: {
    ...
    private func sendCurrentMessage() {
        ...
        // Explicitly clear input text after sending (ensures UI updates immediately)
        viewModel.inputText = ""
    ...
    private func attachScreenshotFromWindow() {
        showWindowSelection = true
    }
    
    private func handleWindowSelected(_ windowID: CGWindowID) async throws {
        // Capture the window screenshot
        let screenshot = try await ScreenshotService.shared.captureWindow(windowID)
        
        // Validate and prepare the image for attachment
        let attachment = try await ImageAttachmentService.shared.validateAndPrepare(
            screenshot,
            preferredFileName: "screenshot.png"
        )
        
        // Attach the screenshot (without auto-sending)
        await MainActor.run {
            viewModel.attachImage(attachment)
        }
    }
