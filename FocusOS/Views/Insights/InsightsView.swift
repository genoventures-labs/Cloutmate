...
struct InsightsView: View {
    ...
    private func handleExport(_ format: InsightsExportFormat) {
        exportWeeklyReflection(as: format)
    }
    
    private func exportWeeklyReflection(as format: InsightsExportFormat) {
        guard let snapshot = currentSnapshot else { return }
        ...
        case .markdown:
            ...
            panel.nameFieldStringValue = "Weekly_Reflection_\(Date().ISO8601Format()).md"
        }
        ...
        panel.begin { response in
            ...
            do {
                ...
                case .pdf:
                    ...
                    guard let data = createPDFData(from: summary) else {
                        Logger.insights.error("Failed to generate PDF data for export")
                        return
                    }
    ...
    private func createPDFData(from text: String) -> Data? {
        ...
        let initialHeight: CGFloat = 792
        let textView = NSTextView(frame: NSRect(x: 0, y: 0, width: pageWidth, height: initialHeight))
        ...
        let usedHeight = textView.layoutManager?.usedRect(for: textView.textContainer!).height ?? initialHeight
        let targetHeight = max(initialHeight, usedHeight + 40)
        textView.frame = NSRect(x: 0, y: 0, width: pageWidth, height: targetHeight)
    ...
    private func buildMemoryGraph() async {
        ...
        let logger = Logger(subsystem: "com.kosmicapps.FocusOS", category: "Insights")
