...
struct MentionDetailPopover: View {
    ...
    private func fetchTask() -> FocusOSShared.Task? {
        let id = mention.id
    ...
    private func fetchProject() -> FocusOSShared.Project? {
        let id = mention.id
    ...
    private func fetchProject(by id: UUID) -> FocusOSShared.Project? {
        let descriptor = FetchDescriptor<FocusOSShared.Project>(
            predicate: #Predicate { $0.id == id }
        )
        return try? modelContext.fetch(descriptor).first
    ...
    private func fetchNote() -> FocusOSShared.Note? {
        let id = mention.id
    ...
    private func fetchPost() -> FocusOSShared.Post? {
        let id = mention.id
    ...
    private func fetchReminder() -> FocusOSShared.Reminder? {
        let id = mention.id
    ...
    private func fetchInboxItem() -> FocusOSShared.InboxItem? {
        let id = mention.id
    ...
    private func fetchArtifact() -> FocusOSShared.Artifact? {
        let id = mention.id
