...
struct AreaDetailView: View {
    ...
    @Query private var allNotes: [FocusOSShared.Note]
    @Query private var allProjects: [FocusOSShared.Project]
    
    var areaTasks: [FocusOSShared.Task] {
        allTasks.filter { $0.areaId == area.id }
    }
    
    var areaNotes: [FocusOSShared.Note] {
        allNotes.filter { $0.areaId == area.id }
    }
    
    var areaProjects: [FocusOSShared.Project] {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            AIRecallService.shared.registerAccessed(area, modelContext: modelContext)
        }
    }
}
...
#Preview {
    AreaDetailView(area: Area(title: "Example Area", notes: "This is a sample area"))
        .modelContainer(for: [Area.self, FocusOSShared.Project.self, FocusOSShared.Task.self, FocusOSShared.Note.self])
}
