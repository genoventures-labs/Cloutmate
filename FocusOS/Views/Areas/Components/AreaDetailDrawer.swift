...
struct AreaDetailDrawer: View {
    ...
    private var linkedProjects: [FocusOSShared.Project] {
    ...
    private var linkedTasks: [FocusOSShared.Task] {
        tasks.filter { $0.areaId == area.id }
    }
    
    private var availableProjects: [FocusOSShared.Project] {
    ...
    private var availableTasks: [FocusOSShared.Task] {
    ...
    private func linkTask(_ task: FocusOSShared.Task) {
        task.areaId = area.id
        task.updatedAt = Date()
        area.updatedAt = Date()
        AreaStabilityService.shared.invalidateCache(for: area.id)
