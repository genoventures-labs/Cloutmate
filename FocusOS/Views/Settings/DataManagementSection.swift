...
struct DataManagementSection: View {
    ...
    private func clearCache() {
        ...
        if let cacheDir = try? fileManager.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: false) {
            do {
                let cacheContents = try fileManager.contentsOfDirectory(at: cacheDir.appendingPathComponent("FocusOS"), includingPropertiesForKeys: nil)
    ...
    private func exportAllData() {
        ...
        panel.nameFieldStringValue = "focusos-export-\(Date().formatted(date: .numeric, time: .omitted)).csv"
