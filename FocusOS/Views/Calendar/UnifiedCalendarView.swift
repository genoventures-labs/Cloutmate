...
struct UnifiedCalendarView: View {
    ...
    @Query(sort: \FocusOSShared.Post.scheduledDate) private var posts: [FocusOSShared.Post]
    @Query(sort: \FocusOSShared.Artifact.publishedAt) private var artifacts: [FocusOSShared.Artifact]
    @Query(sort: \FocusOSShared.Task.dueDate) private var tasks: [FocusOSShared.Task]
    @Query(sort: \CalendarEvent.startDate) private var events: [CalendarEvent]
    
    @State private var selectedDate = Date()
    @State private var isWeeklyView = false
    @State private var activePost: FocusOSShared.Post?
    @State private var activeArtifact: FocusOSShared.Artifact?
    @State private var activeTask: FocusOSShared.Task?
    ...
    @State private var triageRecommendation: TriageRecommendation?
    @State private var lightDayOptimizations: [DayOptimization] = []
    ...
    var body: some View {
        ...
        .task {
            // Start Apple Calendar import service if enabled
            await AppleCalendarImportService.shared.start(modelContext: modelContext)
            
            // Start Apple Reminders import service if enabled
            await AppleRemindersImportService.shared.start(modelContext: modelContext)
            
            // Start Todoist import service if enabled
            await TodoistImportService.shared.start(modelContext: modelContext)
            
            // Start Notion import service if enabled
            await NotionImportService.shared.start(modelContext: modelContext)
            
            // Start Monday.com import service if enabled
            await MondayImportService.shared.start(modelContext: modelContext)
            
            // Check for triage recommendations
            triageRecommendation = cognitionService.checkTriageNeeded(for: selectedDate, modelContext: modelContext)
            lightDayOptimizations = cognitionService.suggestLightDayOptimizations(for: selectedDate, modelContext: modelContext)
        }
        .onChange(of: selectedDate) { newDate in
            triageRecommendation = cognitionService.checkTriageNeeded(for: newDate, modelContext: modelContext)
            lightDayOptimizations = cognitionService.suggestLightDayOptimizations(for: newDate, modelContext: modelContext)
        }
    }
    ...
    private var upcomingOccurrences: [CalendarEventOccurrence] {
        ...
        // Deduplicate events by external ID or by content signature before generating occurrences
        let uniqueEvents = UnifiedCalendarView.deduplicateEvents(events)
        let occurrences = CalendarEventOccurrenceService.shared.occurrences(for: uniqueEvents, in: interval)
        
        // Also deduplicate occurrences by ID to catch any remaining duplicates
        var seenIds = Set<String>()
        let uniqueOccurrences = occurrences.filter { occurrence in
            guard !seenIds.contains(occurrence.id) else { return false }
    ...
    static func deduplicateEvents(_ events: [CalendarEvent]) -> [CalendarEvent] {
        var seenExternalIds = Set<String>()
        var seenSignatures = Set<String>()
        var uniqueEvents: [CalendarEvent] = []
        
        for event in events {
            // First, check by external ID (most reliable for imported events)
            ...
            // For manually created events without external ID, check by signature
    ...
    private var upcomingTasks: [FocusOSShared.Task] {
    ...
    private func upcomingTaskRow(for task: FocusOSShared.Task) -> some View {
        Button {
    ...
    private func update(_ task: FocusOSShared.Task, with draft: TaskDraft) {
    ...
    private func openPostDrawer(for post: FocusOSShared.Post) {
        activePost = post
        postDrawerVisible = true
    }
    
    private func openArtifactDrawer(for artifact: FocusOSShared.Artifact) {
        activeArtifact = artifact
        artifactDrawerVisible = true
    }
    
    private func openTaskDrawer(for task: FocusOSShared.Task) {
        activeTask = task
    ...
    private func mapComposerNotification(_ notification: Notification) -> CalendarComposerRequest {
        ...
        if let post = notification.object as? FocusOSShared.Post {
    ...
    private func itemsForDate(_ date: Date) -> [CalendarDayDetailDrawer.Item] {
        ...
        if activeEntityFilters.contains(.events) {
            ...
            let uniqueEvents = UnifiedCalendarView.deduplicateEvents(events)
            let dayOccurrences = CalendarEventOccurrenceService.shared.occurrences(for: uniqueEvents, in: dayInterval)
                .filter { occurrence in
                    matchesQuery(occurrence.event.title) ||
                    matchesQuery(occurrence.event.notes) ||
                    matchesQuery(occurrence.event.location)
                }
            // Deduplicate occurrences by ID
            var seenOccurrenceIds = Set<String>()
            let uniqueOccurrences = dayOccurrences.filter { occurrence in
                guard !seenOccurrenceIds.contains(occurrence.id) else { return false }
                seenOccurrenceIds.insert(occurrence.id)
                return true
            }
            items.append(contentsOf: uniqueOccurrences.map { CalendarDayDetailDrawer.Item.event($0) })
        }
...
struct UnifiedWeeklyCalendarView: View {
    ...
    let artifacts: [FocusOSShared.Artifact]
    ...
    let onOpenPost: (FocusOSShared.Post) -> Void
    let onOpenArtifact: (FocusOSShared.Artifact) -> Void
    ...
    private func itemsForDate(_ date: Date) -> [CalendarItem] {
        ...
        if activeFilters.contains(.events) {
            ...
            let uniqueEvents = UnifiedCalendarView.deduplicateEvents(events)
            let occurrences = CalendarEventOccurrenceService.shared.occurrences(for: uniqueEvents, in: dayInterval)
                .filter { occurrence in
            ...
            items.append(contentsOf: uniqueOccurrences.map { CalendarItem.event($0) })
        }
...
enum CalendarItem: Identifiable {
    case post(FocusOSShared.Post)
    case artifact(FocusOSShared.Artifact)
    case task(FocusOSShared.Task)
...
struct UnifiedDayColumn: View {
    ...
    let isSelected: Bool
    var onPostClick: ((FocusOSShared.Post) -> Void)?
    var onArtifactClick: ((FocusOSShared.Artifact) -> Void)?
    var onTaskClick: ((FocusOSShared.Task) -> Void)?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            case .event(let occurrence):
                CalendarEventCard(occurrence: occurrence, onTap: {
