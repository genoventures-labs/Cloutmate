...
struct UnifiedNotesView: View {
    ...
    // Sort persistence
    @AppStorage("notes.sort") private var sortOption: String = NoteSortOption.updatedAtDesc.rawValue
    
    private var currentSortOption: NoteSortOption {
        NoteSortOption(rawValue: sortOption) ?? .updatedAtDesc
    }
    ...
    private var sortedNotes: [Note] {
        sortNotes(filteredNotes, by: currentSortOption)
    }
    ...
    @ViewBuilder
    private func quickSortButton(_ sort: NoteSortOption) -> some View {
        Button {
    ...
    @ViewBuilder
    private func sortCategoryMenu(for category: NoteSortCategory) -> some View {
        let options = NoteSortOption.options(for: category)
...
// MARK: - Note Filter Sort Drawer

struct NoteFilterSortDrawer: View {
    @EnvironmentObject private var glassColorSystem: GlassColorSystem
    @Environment(\.colorScheme) private var colorScheme
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    ...
    private var currentSortOption: NoteSortOption {
        NoteSortOption(rawValue: selectedSort) ?? .updatedAtDesc
    }
    ...
    var body: some View {
        V2DrawerScaffold(
            ...
            header: {
                V2GlassHeaderBar(
                    title: "Sort Notes",
                    subtitle: "Refine your note view"
                ) {
                    Button {
                        withAnimation(reduceMotion ? nil : GlassMotion.Easing.modalOpen) {
                            isPresented = false
                ...
                VStack(alignment: .leading, spacing: 32) {
                    ...
                    DrawerSection(title: "Sort Options", icon: "arrow.up.arrow.down.circle") {
                        VStack(alignment: .leading, spacing: 20) {
                            ForEach(NoteSortCategory.allCases, id: \.self) { category in
                                VStack(alignment: .leading, spacing: 12) {
                                    HStack {
                                        Image(systemName: category.icon)
                                            .font(.system(size: 14, weight: .semibold))
                                    ...
                                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 140), spacing: 10)], spacing: 10) {
                                        ForEach(NoteSortOption.options(for: category), id: \.id) { sort in
                                            SortOptionButton(
